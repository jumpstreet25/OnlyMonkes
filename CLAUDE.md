...
- **MonkeBrain is AutonoMonke.** Live fills/alerts are MonkeBrain (MEME vs ESTABLISHED). Classic TA composite is Hermes shadow only — do not buy memes or majors off bipolar composite. **After entry, `tickPositions` still manages the trade** (stop, T1/T2, trail, TA reversal, MonkeBrain thesis-dead). Pump.fun sniper is a separate 0–10m lane. See `Monke_Eliza/agents/monke-trader/src/lib/monkeBrain/` + bot `CLAUDE.md`.
