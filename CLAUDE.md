...
- **Chat LLM chain**: Groq 20b (TPD 429 → 15m) → Gemini 3.6 Flash (429 → 15m) → OpenRouter Llama 3.3 70B (402 → 15m) → **Cloudflare Workers AI** (`@cf/meta/llama-3.1-8b-instruct-fast`) → Ollama **VPS** Qwen3 1.7b → xAI last. DeepSeek R1 is **not** on this chain. Never the Mac Ollama reverse-tunnel (`com.onlymonkes.ollama-tunnel`). Cerebras opt-in. `promptFit.ts` trims per hop (~3 chars/token, Groq 5800). Groq 413 is this-request-only (does not dark Groq for other users). Do **not** put grok-4.6 on this chain.
