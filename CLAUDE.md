...
- **MonkeBrain is AutonoMonke.** Live fills/alerts are MonkeBrain (MEME vs ESTABLISHED). Classic TA composite is Hermes shadow only — do not buy memes or majors off bipolar composite. Pump.fun sniper is a separate 0–10m lane. Stop loss still clamped in the execution engine. See `Monke_Eliza/agents/monke-trader/src/lib/monkeBrain/` + bot `CLAUDE.md`.
