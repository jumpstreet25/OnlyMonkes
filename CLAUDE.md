...
- **Chat LLM chain**: Groq direct (`openai/gpt-oss-20b` / `GROQ_FAST_MODEL`, TPD 200k — 429 opens Groq 15m) → Gemini 3.6 Flash (429 → 15m) → OpenRouter (Llama 3.3 70B; 402 prompt/credits → 15m) → Ollama CPU **Qwen3 1.7b only** → xAI (`grok-4.20-0309-non-reasoning`, 20s, **last on purpose**). DeepSeek R1 1.5B is **not** on this chain. Cerebras opt-in (`CEREBRAS_ENABLED=true`). `promptFit.ts` trims history/system per hop ( Groq 413 / OpenRouter 16k>6560). Circuit breaker 2 failures / 5 min otherwise (`fallbackChain.ts`). Do **not** put grok-4.6 on this chain. Chat replies run detached from the XMTP stream loop.
...
- **VPS has no GPU** (KVM AMD EPYC). Ollama chat hop is CPU-only `qwen3:1.7b`. Do not pull GPU models. Do not route chat through the Mac Ollama tunnel. Full host notes: `Monke_Eliza/agents/monke-trader/VPS.md`.
