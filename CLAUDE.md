...
- **Chat LLM chain**: Groq direct (`openai/gpt-oss-20b` / `GROQ_FAST_MODEL`, TPD 200k — 429 opens Groq 15m) → xAI (`grok-4.20-0309-non-reasoning`, `XAI_API_KEY`) → Gemini 3.6 Flash → OpenRouter (Llama 3.3 70B aggregator) → Ollama local (DeepSeek R1 1.5B + Qwen 3 1.7B). Cerebras is opt-in (`CEREBRAS_ENABLED=true`) — the account is paid-quota (HTTP 402). Model IDs live in `src/lib/llm/models.ts`. Anthropic SDK has been removed from the chain — circuit breaker (2 failures = 5min cooldown per provider) defined in `src/lib/llm/fallbackChain.ts`. Groq retired `llama-3.3-70b-versatile` / `llama-3.1-8b-instant` on 2026-08-16. MonkeBrain alert copy is combinator-only unless `MONKEBRAIN_LLM_COPY=1`. **2026-09-01: `grok-4-1-fast` never existed on this xAI account/plan** (confirmed via `GET /v1/models` — it 404'd silently since whenever that config was written); swapped to `grok-4.20-0309-non-reasoning`, the closest available non-reasoning tier. Do **not** put grok-4.6 on this chain (too slow/expensive for a fallback hop).
