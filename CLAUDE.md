...
- **MonkeBrain is AutonoMonke.** Classifier is automatic: **age < 7d → NEW TOKEN** (safety/liq/vol/flows/holders/mom, fills **0.05 SOL**, HQ+ only). Else MEME vs ESTABLISHED. Classic TA composite is Hermes shadow only — do not buy off bipolar composite. **After entry, `tickPositions` still manages the trade**. Pump.fun sniper is a separate 0–10m lane. **HQ/Legendary fills are not vetoed by user `minConfidence` (HQ 72 used to die on a 75 AI bar).** On `risk_off`, AutonoMonke only fills HQ names with TOKEN/SOL 24h > 0; Conditional is skipped. Future Pro: NewTokenBrain / MemeBrain / AltBrain inside MonkeBrain. See `Monke_Eliza/agents/monke-trader/src/lib/monkeBrain/` + bot `CLAUDE.md`.
...
- **Chat LLM chain**: Groq direct (`openai/gpt-oss-20b` / `GROQ_FAST_MODEL`, TPD 200k — 429 opens Groq 15m) → xAI **Grok 4.1 Fast only** (`grok-4-1-fast`, `XAI_API_KEY`) → Gemini 3.6 Flash → OpenRouter (Llama 3.3 70B aggregator) → Ollama local (DeepSeek R1 1.5B + Qwen 3 1.7B). Cerebras is opt-in (`CEREBRAS_ENABLED=true`) — the account is paid-quota (HTTP 402). Model IDs live in `src/lib/llm/models.ts`. Anthropic SDK has been removed from the chain — circuit breaker (2 failures = 5min cooldown per provider) defined in `src/lib/llm/fallbackChain.ts`. Groq retired `llama-3.3-70b-versatile` / `llama-3.1-8b-instant` on 2026-08-16. MonkeBrain alert copy is combinator-only unless `MONKEBRAIN_LLM_COPY=1`. Do **not** put grok-4.6 on this chain.
