...
## Mission — never stop improving (standing directive, 2026-09-14)

**Never stop improving MonkeBrain, AutonoMonke, bot functions, the published OnlyMonkes dApp, and future published dApps.** Proactively surface improvement areas — don't wait to be asked. Mirrored verbatim in `Monke_Eliza/CLAUDE.md`; keep both in lockstep.

**Weekly research is the standing expectation, not optional** — once a week (target: Friday), walk both repos for what needs attention:
1. MonkeBrain (score, universes, fills, sizing, vetoes, Hermes/BrainVsTa win rate)
2. AutonoMonke engine (gates, TA vs Brain books, copy-trade, pump lane)
3. Bot ops (XMTP streams, DMs, HTTP :3001, join flow, `/command`s)
4. The published dApp (OTA-only 3.4: chat, Trades, Genesis, marketplace, ads, watchlist)
5. Rank 5–10 next improvements — reliability first, then fill quality, then dApp. Skip theater: no MLS group recreate, no global `XMTP_IDENTITY_DOMAIN` bump, no native/Gradle changes on master, no grok-4.6 on the chat chain, no BananaBetting resurrection.
6. Deliver a short "do this week" list — research only unless explicitly told to build.

**Not yet automated.** The plan is a GitHub Action cron opening a labeled `weekly-research` issue that `/claude`/`/grok` (admin DM → `src/lib/claudeFixIssue.ts`) can pick up — but that whole DM path is blocked on `GITHUB_ISSUES_TOKEN` not being set on the VPS (see "Remote code-fix via bot DM" below), so nothing here runs unattended yet. Until that's unblocked, this research happens inside an active session — prompted or unprompted, per the first line above.

**Do not ship a second XMTP bot process to do this.** Admin DMs to the existing "AI Agent #9385" identity are enough — see the XMTP & Messaging rules below for why a second installation/process is never the answer to any of this.
...
- The token scanner (`tokenScanner.ts`) scans the discovery universe every 10 min. OHLCV chain (80%-of-limit budgets, sliding windows/backoff per provider): GeckoTerminal (24/min, never wait for a slot) → Vybe Network (4/min) → DexPaprika (~8k/day, no auth) → **Moralis Solana OHLCV re-added 2026-08 — now supported, 40K CU/day free (100 calls/cycle budget)** → TradingStrategy (200/min, Solana support uncertain) → PumpFun → Birdeye (daily-CU, last resort). DexScreener batch-enriches pricing/pool-resolution first (free, warms cache). **PumpFun/some DexPaprika candles are TOKEN/USD while `token.price` is TOKEN/SOL.** Rescale USD→SOL with `applyUsdSolRescale` from the token's own SOL price — **do not use Pyth SOL/USD** (SSE 401 left KMNO-only coverage, 2026-08-26). Real mismatches (wrong pool, 1e9×) still skip. Live book is MonkeBrain; classic TA is Hermes shadow only. **PYTH token is blocked from AutonoMonke/MonkeBrain alerts & fills; `/chart` `/ta` still work.**
...
- **Chat LLM chain**: Groq 20b then Groq 120b on 429/TPD (per-model 200k; 413 skips 120b) → Gemini 3.6 Flash (429 → 15m) → OpenRouter Llama 3.3 70B (402 → 15m) → **Cloudflare Workers AI** (`@cf/meta/llama-3.1-8b-instruct-fast`) → Ollama **VPS** Qwen3 1.7b → xAI last. DeepSeek R1 and Groq Qwen 27b are **not** on this chain. Never the Mac Ollama reverse-tunnel (`com.onlymonkes.ollama-tunnel`). Cerebras opt-in. `promptFit.ts` trims per hop (~3 chars/token, Groq 5800). Groq 413 is this-request-only. Do **not** put grok-4.6 on this chain. **Background scanning** (name vet / multi-perspective / position review / Sunday tune) is **VPS Ollama only**: `llama3.2:3b` with `qwen3:1.7b` fallback — $0, no Groq. `qwen3.5:4b` is too slow on the 6 vCPU VPS.
...
- **VPS has no GPU** (KVM AMD EPYC). Ollama chat hop is CPU-only `qwen3:1.7b` **on the VPS**. Do not pull GPU models. Do not run `ollama serve` or `com.onlymonkes.ollama-tunnel` on the Mac. Full host notes: `Monke_Eliza/agents/monke-trader/VPS.md`.
...
- **Bot channel bar is MonkeTrades only.** Bets / Predictions / Sales icons stay off the composer. NFT sales alerts are merged into the Trades group (`salesGroup = tradesGroup`). Apply on **both** runtimes: 3.3 production OTA from `ota-3.3` worktree; **3.4 is also channel `production`, published straight from `master`** (corrected 2026-09-16 — verified via `eas build:list`: the last successful runtime-3.4 native build, commit `d118d8386`, is channel `production` with `react-native-reanimated: ~4.3.4`, matching master's current dependency exactly. The `OnlyMonkes-ota-3.4` worktree/branch `ota-3.4-fail-closed` is a stale, narrow task branch from an earlier fix — missing later screens entirely (e.g. `app/treasury.tsx` doesn't exist there), `node_modules` never installed, not a maintained general-purpose 3.4 staging branch. Don't publish from it; before any `eas update --branch production`, re-verify with `eas build:list --platform android --limit 1` + `git show <that commit>:package.json | grep reanimated` against current `master` — don't trust a worktree name alone). Do not resurrect the 4-icon bar. **2026-09-03: sale delivery had a silent-drop bug** — both the bot's direct-to-bot Helius webhook (`POST /helius-webhook`, port 3001, publicly bound) and its 180s polling safety-net (`sagaMonkesSales.ts`, single creator wallet — royalties always pay on Tensor so coverage wasn't the issue) used to mark a sale seen/persisted *before* confirming the send succeeded, so a transient failure on either path blackholed the sale forever with zero retry. Both now only mark/persist after a confirmed send.
