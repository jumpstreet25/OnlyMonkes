...
### AI Agent & TA Scanner
- **AI Agent #9385 (Monke)** — unified XMTP bot identity with a confident, ball-busting, banana-obsessed persona; powers all features: TA scanning, trade alerts, NFT sales, DM commands, and LLM chat; own codebase (ElizaOS removed 2026-09-16 — it was boot-wrapper only), trades execute directly via Jupiter; Pyth Hermes SSE streaming for sub-second price feeds
- **Multi-LLM brain chain** — Groq `openai/gpt-oss-20b` then `openai/gpt-oss-120b` on 429/TPD → Gemini 3.6 Flash → OpenRouter (Llama 3.3 70B) → **Cloudflare Workers AI** (`@cf/meta/llama-3.1-8b-instruct-fast`) → Ollama **CPU-only last resort** on the VPS (`qwen3:1.7b` only — DeepSeek R1 is not on this chain) → xAI `grok-4.20-0309-non-reasoning` **last, on purpose** (the only paid-per-token step — every free/local option is tried first). **The VPS has no GPU** — see `Monke_Eliza/agents/monke-trader/VPS.md` (keep that file updated; Claude and Grok both read it). Model IDs live in `Monke_Eliza/agents/monke-trader/src/lib/llm/models.ts`. Hermes *memory* (`hermesMemory.ts`) is injected into the prompt; the Hermes CLI is **not** in the per-message chain. Cerebras is opt-in only (`CEREBRAS_ENABLED=true` on the VPS) — that account is paid-quota (HTTP 402). Groq retired `llama-3.3-70b-versatile` / `llama-3.1-8b-instant` on 2026-08-16. Anthropic is removed. Circuit breaker: 2 failures → 5 min cooldown per provider (`fallbackChain.ts`). Groq 413 (prompt too large) skips that message only and does not dark Groq. `promptFit.ts` trims per hop (~3 chars/token, Groq budget 5800). **Production bot is VPS-only** (`systemd` `monketrader.service` on `monke@157.173.192.39`). Do not start the Mac LaunchAgent — a second process locks the XMTP DB. Ollama fallback timeout is 30s; chat replies run detached from the XMTP stream loop so one slow reply can't block other users' messages.
...
- **AutonoMonke / MonkeBrain** — live book is **MonkeBrain** (NEW TOKEN <7d at **0.05 SOL**, MEME, ESTABLISHED). Alerts + fills at **MonkeScore ≥ 69** on any tape (`risk_on`/`off` is not an entry gate). Classic TA composite still scores every cycle (`brainVsTa`); it does **not** open fills. TA composite **60–69** near-misses may post as `TA SHADOW — no fill` in MonkeTrades (max 2/cycle). Pump.fun 0–10m sniper is a third lane. Sunday Hermes 7-day tune.
...
### Bot Platform Notes
- **NFT sale images** — Saga Monkes sales alerts include the actual NFT artwork fetched via Helius DAS API, posted to **MonkeTrades** (Sales merged into Trades 2026-08-13) and as a big-picture push notification. Composer bar shows **one** channel button on 3.3 and 3.4. (2026-09-03: fixed a silent-drop bug where a sale could be marked delivered before the send was actually confirmed, permanently skipping retry — see bot `CLAUDE.md`/`VPS.md`.)
