...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/api/community/luma-events") {
      if (request.method === "GET") return handleGetLumaEvents(env);
      return errorResponse("Method not allowed", 405);
    }
