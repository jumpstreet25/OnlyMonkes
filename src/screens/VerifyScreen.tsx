...
type VerifyState = "idle" | "checking-nft" | "nft-fail" | "verify-error" | "nft-ok" | "pick-nft" | "ready";
...
export default function VerifyScreen() {
  ...
  // 2026-07-26: root-caused a real production report — a legitimate NFT
  // holder on the published app landed on the retry-verification screen.
  // This effect used to fire once on mount with `[]` deps regardless of
  // wallet state — if the wallet-connect promise resolved even slightly
  // after this screen mounted, the ONE AND ONLY verification attempt ran
  // with a stale null wallet, hit "No wallet connected", and never
  // automatically retried once wallet actually became available (empty
  // deps array never re-fires). The user had to manually tap Retry, by
  // which point wallet was set and it just worked — exactly the confusing
  // "retry screen, but retry works" shape reported. Now gated on
  // wallet.address itself: never fires on a null wallet, and automatically
  // re-fires the moment it becomes available, closing the race instead of
  ...
  const runVerification = async () => {
    ...
    if (!result.verified) {
      // Same as 3.4: an outage is not "you're not a holder". 3.3 used to
      // dump those users into guest/marketplace with no retry path.
    ...
    await goToChat(fresh.verifiedNft ?? undefined);
  };
