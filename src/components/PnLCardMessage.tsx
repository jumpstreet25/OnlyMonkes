import React from 'react';
import { View, Text, Pressable, StyleSheet } from 'react-native';
import { THEME, FONTS } from '@/lib/constants';
import type { ClosedTrade } from '@/lib/positions';

interface PnLCardMessageProps {
  trade: ClosedTrade;
  onPress: (trade: ClosedTrade) => void;
}

function formatDuration(ms: number): string {
  const sec = Math.floor(ms / 1000);
  const days = Math.floor(sec / 86400);
  const hours = Math.floor((sec % 86400) / 3600);
  const mins = Math.floor((sec % 3600) / 60);
  if (days > 0) return `${days}d ${hours}h`;
  if (hours > 0) return `${hours}h ${mins}m`;
  return `${mins}m`;
}

export function PnLCardMessage({ trade, onPress }: PnLCardMessageProps) {
  const isWin = trade.pnlPct >= 0;
  const accent = isWin ? THEME.gold : THEME.error;
  const sign = isWin ? '+' : '';

  return (
    <Pressable
      onPress={() => onPress(trade)}
      style={({ pressed }) => [styles.row, pressed && { opacity: 0.7 }]}
    >
      <View style={[styles.bubble, { borderColor: accent + '55' }]}>
        <View style={styles.header}>
          <Text style={[styles.label, { color: accent }]}>
            {trade.source === 'autonomonke' ? '🤖 AUTONOMONKE CLOSED' : '🍌 TRADE CLOSED'}
          </Text>
        </View>
        <View style={styles.middle}>
          <Text style={[styles.pct, { color: accent }]}>
            {sign}{trade.pnlPct.toFixed(2)}%
          </Text>
          <Text style={styles.token}>${trade.token.toUpperCase()}</Text>
        </View>
        <View style={styles.footer}>
          <Text style={styles.meta}>{formatDuration(trade.durationMs)}</Text>
          <Text style={styles.cta}>Tap to share →</Text>
        </View>
      </View>
    </Pressable>
  );
}

const styles = StyleSheet.create({
  row: { paddingHorizontal: 12, paddingVertical: 4, alignItems: 'flex-end' },
  bubble: {
    minWidth: 220,
    maxWidth: '85%',
    borderRadius: 16,
    borderWidth: 1,
    backgroundColor: THEME.surfaceHigh,
    paddingVertical: 10,
    paddingHorizontal: 14,
    gap: 4,
  },
  header: { flexDirection: 'row', alignItems: 'center' },
  label: { fontFamily: FONTS.mono, fontSize: 10, letterSpacing: 1.2 },
  middle: { flexDirection: 'row', alignItems: 'baseline', gap: 8 },
  pct: { fontFamily: FONTS.display, fontSize: 22, letterSpacing: -0.5 },
  token: { fontFamily: FONTS.mono, fontSize: 13, color: THEME.text },
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  meta: { fontFamily: FONTS.body, fontSize: 11, color: THEME.textMuted },
  cta: { fontFamily: FONTS.bodyMed, fontSize: 11, color: THEME.textMuted },
});
